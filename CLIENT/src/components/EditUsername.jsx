...
function EditUsername() {
  ...
  const handleSubmit = async (event) => {
    ...
    const { username, email, password, newUsername } = formData;

    try {
      if (!username || !email || !password || !newUsername) {
      ...
      const response = await axios.put("http://localhost:8000/api/v1/users/username", formData, {
        headers: {
  ...
  return (
    <div className="container mt-5 bg-light border rounded p-4 shadow-sm shadow-lg p-3 mb-5 bg-body-tertiary rounded" style={{ maxWidth: "25rem", maxHeight: "100vh" }}>
      ...
      <form id="register-form" onSubmit={handleSubmit}>
        ...
        <Input label="New Username" type="text" name="newUsername" value={formData.newUsername} onChange={handChange} />
